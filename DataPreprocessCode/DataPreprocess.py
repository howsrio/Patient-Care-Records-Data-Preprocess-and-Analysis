...
class DataPreprocess:
    ...
    def __init__(self, region):
        ...
        self.path = os.path.dirname(self.path) # 2단계 이전의 폴더를 기본 경로로 지정
        self.path= self.path + f'\\region\\{region}'
        self.region = region
        
    def makeDir(self):
        self.newPath = os.path.join(f'{self.path}\\PreprocessData') 
         #이미 존제할 경우 발생하는 오류를 막음
    ...
    def makeInbody(self):
        ...
        inbody_file_names = [filename for filename in self.fileList if isinstance(filename, str) and 'inbody' in filename.lower()] #파일 명중에 inbody가 포함된 파일 이름 저장

        # 다음 3가지 자료는 별도에 가공 과정이 필요하여 분리
